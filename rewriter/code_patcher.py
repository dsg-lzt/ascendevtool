...
def _generate_replacement_module(output_dir: Path, solutions: List[OpSolution]) -> Path:
    ...
    lines = [
        ...
        "        B, C, N = xyz.shape",
        "        idx_expand = idx.unsqueeze(1).expand(B, C, *idx.shape[1:]) if idx.dim() == 2 else idx",
        "        return torch.gather(xyz, 2, idx_expand) if idx.dim() == 2 else xyz.reshape(B, C, -1)[:, :, idx.reshape(B, -1)].reshape(B, C, *idx.shape[1:])",
        "if _mmcv_grouping_operation is None:",
        "    def _mmcv_grouping_operation(xyz, idx):",
        "        B, C, N = xyz.shape",
        "        return xyz.view(B, C, -1)[:, :, idx.view(B, -1)].view(B, C, *idx.shape[1:])",
