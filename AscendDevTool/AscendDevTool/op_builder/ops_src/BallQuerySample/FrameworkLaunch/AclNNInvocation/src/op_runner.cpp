...
bool OpRunner::RunOp()
{
    ...
    constexpr float kMinRadius = 0.15f;
    constexpr float kMaxRadius = 0.45f;
    constexpr int64_t kSampleNum = 8;

    size_t workspaceSize = 0;
    aclOpExecutor *handle = nullptr;

    auto ret = aclnnBallQueryGetWorkspaceSize(
        inputTensor_[0],
        inputTensor_[1],
        kMinRadius,
        kMaxRadius,
    ...
    ret = aclnnBallQuery(workspace, workspaceSize, handle, stream);
    if (ret != ACL_SUCCESS)
    {
        (void)aclrtDestroyStream(stream);
    ...
    INFO_LOG("Execute aclnnBallQuery success");
