...
{
    // BallQuery(xyz, center_xyz, min_radius, max_radius, sample_num) -> idx
    std::vector<int64_t> xyzShape{64, 3};
    std::vector<int64_t> centerShape{16, 3};
    constexpr int64_t sampleNum = 8;
    std::vector<int64_t> outputShape{centerShape[0], sampleNum};

    aclDataType inputType = ACL_FLOAT16;
    aclDataType outputType = ACL_INT32;
    aclFormat format = ACL_FORMAT_ND;

    OperatorDesc opDesc;
    opDesc.AddInputTensorDesc(inputType, xyzShape.size(), xyzShape.data(), format);
    opDesc.AddInputTensorDesc(inputType, centerShape.size(), centerShape.data(), format);
    opDesc.AddOutputTensorDesc(outputType, outputShape.size(), outputShape.data(), format);
    return opDesc;
}
...
bool SetInputData(OpRunner &runner)
{
    ...
    ReadFile("../input/input_center_xyz.bin", fileSize, runner.GetInputBuffer<void>(1), runner.GetInputSize(1));
