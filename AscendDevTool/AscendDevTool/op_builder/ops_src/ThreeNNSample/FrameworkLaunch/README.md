## 概述
本样例基于FastGelu算子工程，介绍了单算子调用的调用方式。
## 目录结构介绍
``` 
├── FrameworkLaunch           //使用框架调用的方式调用FastGelu算子
│   ├── AclNNInvocation       // 通过aclnn调用的方式调用FastGelu算子
│   ├── FastGelu              // FastGelu
│   └── FastGelu.json         // FastGelu算子的原型定义json文件
``` 
## 算子工程介绍
算子工程目录FastGelu包含算子实现的模板文件、编译脚本等，如下所示:
``` 
├── FastGelu                //FastGelu自定义算子工程
│   ├── cmake
...
``` 
CANN软件包中提供了工程创建工具msopgen，FastGelu算子工程可通过FastGelu.json自动创建，具体请参考[Ascend C算子开发](https://hiascend.com/document/redirect/CannCommunityOpdevAscendC)>算子开发>算子开发工程>基于自定义算子工程的算子开发>创建算子工程 章节。
## 编译运行样例算子
...
### 2.&nbsp;编译算子工程<a name="operatorcompile"></a>
  ...
  - 修改CMakePresets.json中ASCEND_CANN_PACKAGE_PATH为CANN软件包安装后的实际路径。
    ...
    ```
  - 在算子工程FastGelu目录下执行如下命令，进行算子工程编译。
...
### 4.&nbsp;配置环境变量
  ...
  - 指定路径install_path，安装CANN软件包
    ...
    ```
### 5.&nbsp;调用执行算子工程
- [aclnn调用FastGelu算子工程](./AclNNInvocation/README.md)
