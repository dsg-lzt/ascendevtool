...
private:
    __aicore__ inline void ProcessBatch(uint32_t b) {
        ...
        pipe.InitBuffer(qX, BUF_NUM, bufN_ * sizeof(T));
        pipe.InitBuffer(qY, BUF_NUM, bufN_ * sizeof(T));
        pipe.InitBuffer(qZ, BUF_NUM, bufN_ * sizeof(T));
        pipe.InitBuffer(mdBuf,   N_ * sizeof(T));
        pipe.InitBuffer(bufDist, bufN_ * sizeof(T));
        pipe.InitBuffer(bufTmp,  bufN_ * sizeof(T));
        pipe.InitBuffer(bufSca,  bufN_ * sizeof(T));
    ...
    uint32_t B_, N_, M_, tileN_, bufN_, start_, end_;
    T initVal_;
};
