...
public:
    ...
    __aicore__ inline void Init(GM_ADDR p, GM_ADDR s,
        ...
        if (static_cast<uint32_t>(blockIdx) < crem) {
            start_ = blockIdx * static_cast<int32_t>(bpc + 1);
            end_ = start_ + static_cast<int32_t>(bpc) + 1;
        } else {
            start_ = static_cast<int32_t>(crem * (bpc + 1) +
                     (blockIdx - static_cast<int32_t>(crem)) * bpc);
